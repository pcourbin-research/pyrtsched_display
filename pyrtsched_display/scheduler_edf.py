...
class SchedulerEDF(Scheduler):
    def _next_job_scheduler(self, resource: Resource, current_time: int, jobs_to_execute: list[str]) -> str: # Job name selected for execution on the resource
        ...
        for job_name in jobs_to_execute:
            ...
            if ((min_deadline == -1 or job_deadline < min_deadline or (job_deadline == min_deadline and job_activation < job_selected_activation)) and job_resource_type == resource_type):
                min_deadline = job_deadline
